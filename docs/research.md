...
Aggregates 1-minute OHLCV data into daily metrics. Thin wrapper around `qqq_trading.data.daily_metrics.build_daily_metrics()`.

> 将1分钟OHLCV数据聚合为日线指标。是`qqq_trading.data.daily_metrics.build_daily_metrics()`的薄封装。
